...
# Read data
path_data = paste0(PATH, 'windspeed_data_cgm_std.feather')
...
objective_fun = function(par, ensmean, ensvar, obs){
  ...
  if(any(ssq_tmp < 0)){
    ...
    s = sqrt(ssq_tmp)
    return(sum(crps_tnorm(y = obs, location = m, scale = s, lower = 0)))
  }
}
...
gradfun_wrapper = function(par, obs, ensmean, ensvar){
  ...
  dcrps_dtheta = gradcrps_tnorm(y = obs, location = loc, scale = sc, lower = 0) 
...
postproc_local = function(vdate, train_length, data){
  ...
  for(this_station in stations_list){
    ...
    optim_out = optim(par = c(1,1,1,1), 
                       fn = objective_fun,
                       gr = gradfun_wrapper,
                       ensmean = data_train$ws_mean, 
                       ensvar = data_train$ws_var, 
                       obs = data_train$obs,
                       method = "BFGS")
...
for (n in 1:an) {
  ...
  for(day_id in 1:length(eval_dates)){
    ...
    for(this_station in stations_list){
      ...
      loc_st = c(cbind(1, data_eval$ws_mean) %*% par_output[ind_st,1:2])
      scsquared = c(cbind(1, data_eval$ws_var) %*% par_output[ind_st,3:4])
      ...
      EMOS_sample = qtruncnorm(qlevels, a=0, mean=loc_st, sd=sc_st)
  ...
  train_obs_all_long = ens_fc_ws_subset %>%
  ...
  for(day_id in 1:length(train_obs_dates)){
    ...
    data_today = subset(ens_fc_ws_mv_subset, date == today)
    ...
    for(this_station in stations_list){
      ...
      if(latent_obs == Inf) {latent_obs = 8.209536} # to avoid Inf
  ...
  for(day_id in 1:length(eval_dates)){
    ...
    mvsample = mvrnorm(n = 50, mu = rep(0,dim), Sigma = corr_obs)
    ...
    for(this_station in stations_list){
      ...
      mvpp_thisstation = 
        qtruncnorm(pnorm(mvsample[,ind_st]), 
                   a=0, mean = loc_st, sd = sc_st)
