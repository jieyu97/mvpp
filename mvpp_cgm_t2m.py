...
for k in range(n_rep):
    ...
    # Inputs
    x_training = [np.concatenate((norm_ens_mu[train_dateindex].values.reshape((-1, dim, 1)),
                                  norm_addvar_mu[train_dateindex].values.reshape((-1, dim, n_add_variable))
                                  ), axis=-1),
                np.concatenate((norm_ens_sigma[train_dateindex].values.reshape((-1, dim, 1)),
    ...
    x_validation = [np.concatenate((norm_ens_mu[val_dateindex].values.reshape((-1, dim, 1)),
                                    norm_addvar_mu[val_dateindex].values.reshape((-1, dim, n_add_variable))
                                    ), axis=-1),
                    np.concatenate((norm_ens_sigma[val_dateindex].values.reshape((-1, dim, 1)),
    ...
    x_test = [np.concatenate((norm_ens_mu[test_dateindex].values.reshape((-1, dim, 1)),
                              norm_addvar_mu[test_dateindex].values.reshape((-1, dim, n_add_variable))
                              ), axis=-1),
            np.concatenate((norm_ens_sigma[test_dateindex].values.reshape((-1, dim, 1)),
    ...
    y_training = norm_obs[train_dateindex].values.reshape((-1, dim, 1))
    y_validation = norm_obs[val_dateindex].values.reshape((-1, dim, 1))
    y_test = norm_obs[test_dateindex].values.reshape((-1, dim, 1))
    ...
    for loop in range(5):
        ...
        # inverse normalization of output t2m ensemble predictions
        fcst = scaler.inverse_transform(np.reshape(pre_data, (pre_data.shape[0]*pre_data.shape[1],-1)))
